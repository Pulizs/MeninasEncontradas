...
// --- CONSTANTS (EDITE AQUI OS DADOS DA CRIANÇA) ---
const CHILD_DATA: ChildProfile = {
  name: "Filha", // Mude para o nome real
  age: "6 anos",
...
const LocationRequest: React.FC<{ onLocationFound: (url: string) => void }> = ({ onLocationFound }) => {
  ...
  const handleGetLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      (error) => {
        setStatus('error');
        setErrorMsg('Erro ao obter localização. Permita o acesso.');
      },
    ...
    return (
      <div className="bg-green-50 border border-green-200 rounded-lg p-3 mb-6 flex items-center gap-3 animate-in fade-in slide-in-from-top-2">
        ...
        <p className="text-sm text-green-800 font-medium">
          Localização anexada ao WhatsApp.
        </p>
      </div>
    );
  }
    ...
    <div className="mb-6">
      ...
      >
        {status === 'loading' ? (
          ...
          <>
            <MapPin size={20} />
            {status === 'error' ? 'Tentar novamente' : 'Anexar minha localização atual'}
          </>
        )}
      </button>
      {status === 'idle' && (
        <p className="text-xs text-center text-gray-500 mt-2">
          Toque acima para enviar sua posição exata junto com a mensagem.
        </p>
      )}
      {status === 'error' && (
